...
void loadVirtualFileCallback(const char *contents, size_t length, void* encodedData) {
    [(NSData *)encodedData initWithBytes:contents length:length];
}
...
readContentsOfFile(NSString* path, void** buf, unsigned int* len, NSZone* zone)
{
    ...
    if ([path hasPrefix:NSHomeDirectory()]) {
        ...
        NSData *encodedData = [NSData alloc];
        result = VerdePluginLoadVirtualFile([path cString], loadVirtualFileCallback, encodedData);
        if (result) {
            NSData *decodedData = [GSMimeDocument decodeBase64:encodedData];
            length = [decodedData length];
            buffer = (char*)malloc(length);
            [decodedData getBytes:buffer];
        }
    } else {
...
writeContentsToFile(NSString* path, NSData *data)
{
    if ([path hasPrefix:NSHomeDirectory()]) {
        ...
        return VerdePluginWriteVirtualFile([path cString], [encodedData bytes], [encodedData length]);
    } else {
        return VerdePluginWriteFile([path cString], [data bytes], [data length]);
    }
}
